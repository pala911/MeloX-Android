...
class LxUserPlaybackResolver(
     ...
     *
     * Upstream 0.6.1 wraps this in an in-flight de-duplication map keyed by
     * `source:id:quality`. We deliberately do not: a prefetching caller would then
     * hand its future to the track the user just tapped, which defeats the urgent
     * priority queue in LxUserRuntimeSession and can park the playback coroutine for
     * up to 45 s — far past our per-track resolve budget. LxUserRuntimeSession already
     * serialises every call through one runtime and one rate-limit slot.
     */
    internal fun resolve(
        track: MusicTrack,
        quality: AudioQualityTier,
